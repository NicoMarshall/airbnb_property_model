...
def tune_hyperparameters(model_class, features, labels, hyperparam_dict):
    ...
    for i in hyperparam_combs:
        try:
            model = model_class(**i)
            model.fit(x_train,y_train)
            mod_metrics = evaluate_model(model, x_test, y_test, label_list)
            if mod_metrics[0] > best_test_accuracy:
                best_test_accuracy = mod_metrics[0]
                best_hyperparams = i
                best_model = model
                best_model_metrics = mod_metrics
            else:
                pass
        except:
            ValueError
...
if __name__ =="__main__":
    ...
    x_train ,x_test ,y_train ,y_test = model_selection.train_test_split(features,labels, test_size=0.3)
    x_validation, x_test, y_validation,y_test = model_selection.train_test_split(x_test,y_test,test_size = 0.5)
    model_path = f"C:\\Users\\nicom\\OneDrive\\Υπολογιστής\\airbnb_property_model\\models\\classification\\random_forest\\model.joblib"
    model_1 = joblib.load(model_path)
    
    acc = evaluate_model(model_1, x_test, y_test, label_list)[0]
    print(acc)
    
    y_pred = model_1.predict(x_test)
    
    accuracy_score_1 = metrics.accuracy_score(y_test,y_pred)
